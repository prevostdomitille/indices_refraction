...
def transmission(angle, delta_n, angle_interface):
    ...
    T3, _ , angle3, _ = interface(DegreeAngle(90 - angle_interface.deg() - angle2.deg()), n3, n3 + delta_n)
    T4, angle4 = passage_couche(DegreeAngle(angle_interface.deg() - angle3.deg()), n3 + delta_n, n4, n5, nb_iterations)
    # T5, _ = passage_couche(angle4, n4, n5, n1, nb_iterations)
    transmission = T1 * T2  * T4
